<?php
 ...
 /* $Id: imdb_person.class.php 429 2010-12-01 16:20:49Z izzy $ */
  ...
  * @version $Revision: 429 $ $Date: 2010-12-01 17:20:49 +0100 (Mi, 01. Dez 2010) $
  */
 ...
 #-----------------------------------------------------------[ Constructor ]---
  ...
  function __construct($id) {
    ...
    $this->revision = preg_replace('|^.*?(\d+).*$|','$1','$Revision: 429 $');
    $this->setid($id);
  }
  ...
  private function filmograf(&$res,$type) {
    ...
    if (preg_match_all('!<div class="filmo-row.*?>\s*(.*?)\s*<div!ims',$match[1],$matches)) {
      ...
      for ($i=0;$i<$mc;++$i) {
        ...
        $str = $matches[1][$i]; //preg_replace('|\(\d{4}\)|','',substr($matches[4][$i],0,strpos($matches[4][$i],"<br>")));
